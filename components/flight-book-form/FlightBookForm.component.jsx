...
const FlightBookForm = ({ travData, grandTotal, taxes }) => {
  ...
  const flightBookSubmit = async () => {
    ...
    try {
      ...
      const res = axios.post(`${api_url}/send-mail`, dataToSend);
      ...
      >
        ...
        <tbody>
          ...
          <tr>
            ...
            <td
              currency="value"
              className="cm-wt-700 cm-sec-col2"
              fare={grandTotal}
            >
              $ {grandTotal}
            </td>
          ...
          <tr>
            <th colSpan={3} style={{ fontWeight: '400' }}>
              Tax (<span currency="code">USD</span>)
            </th>
            <td currency="value" className="cm-wt-700 cm-sec-col2" fare={taxes}>
              $ {taxes}
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <th colSpan={3}>
              Final Total Price: (<span currency="code">USD</span>)
            </th>
            <td currency="value" className="cm-gd-total cm-wt-700 cm-sec-col2">
              $ {calcFinalPrice()}
            </td>
          </tr>
        </tbody>
  ...
  return (
    <div className="cm-section cm-fl-book-form">
      {/* <div className="cm-form-section cm-pass-contact-info">
        <div className="cm-form-head cm-prim-bg2">
      ...
      <div className="cm-form-section">
        <div className="cm-form-head cm-prim-bg">
      ...
      <div className="cm-form-section">
        <div className="cm-form-head cm-prim-bg">
          <h3 className="cm-section-sh">
            Add Baggage Now, Save Time and Money! (Optional)
          </h3>
        </div>
        <div className="cm-form-field-grp">
          <div className="cm-form-field">
            <label>
              Baggage allowance / fees (Checked Bag - Includes Delayed Baggage
              Protection)
            </label>
            <div className="cm-multi-checkbox cm-flex">
              {charges.baggage.map((el, index) => (
                <div key={index} className="cm-checkbox-wrap cm-flex">
                  <input
                    type="checkbox"
                    name="baggage"
                    value={`${index + 1} Bag${index + 1 > 1 ? 's' : ''
                      } @ $${el}`}
                    onChange={(e) => handleCheckChange(e, 'baggage')}
                    checked={formVal.baggage.includes(
                      `${index + 1} Bag${index + 1 > 1 ? 's' : ''} @ $${el}`
                    )}
                  />
                  <label>
                    {index + 1} Bag{index + 1 > 1 ? 's' : ''} - ${el}
        ...
        <div className="cm-baggage-total-wrap cm-flex-type-1">
          <p>Total Baggage Price:</p>
          <h4>$ {calcTotalBaggagePrice()}</h4>
        </div>
      </div>

      <div className="cm-form-section">
        <div className="cm-form-head cm-prim-bg">
          <h3 className="cm-section-sh">Travel Protection (Recommended)</h3>
        </div>

        <div className="cm-travel-protection-wrap cm-flex-type-1">
          <div className="cm-col cm-col1">
            <img src={travProtection} alt="Travel Protection" />
          </div>
          <div className="cm-col cm-col2">
            <h4>Reasons you might need travel protection</h4>
            <ul className="cm-flex cm-flex-wrap">
        ...
        <div className="cm-form-field-grp">
          <div className="cm-form-field">
            <div className="cm-radio-grp">
              <div className="cm-radio-wrap cm-flex">
                <input
                  type="radio"
                  name="travelProtection"
                  value={true}
                  onChange={(e) => handleRadioChange('travelProtection', true)}
                  checked={formVal.travelProtection}
                />
                <label>
                  Yes, I want Travel Protection for my trip. + $
                  {charges.travelProtection}
                </label>
              </div>
              <div className="cm-radio-wrap cm-flex">
                <input
                  ...
                  name="travelProtection"
                  value={false}
                  onChange={(e) => handleRadioChange('travelProtection', false)}
                  checked={!formVal.travelProtection}
                />
                <label>No, I'm willing to risk my trip.</label>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="cm-form-field-grp cm-pay-info-wrap">
          <div className="">
            <div className="cm-form-field">
              ...
              <select onChange={(e) => handleFieldChange(e, 'paymentMethod')}>
                ...
                <option value="paypalcredit">Paypal Credit Card</option>
              </select>
            </div>
            <div className="cm-form-field">
              <label>
      ...
      <div className='cm-price-detail'>
        <div className='price-heading'>
          <h3>Price Detail (USD)</h3>
        </div>
        <div className='price-detail-content'>
          {/* <p>Please confirm that the dates and times of your flights are correct: Depart (Wed, Jul 18) . This date is an alternate date.</p> */}
          <p>Please also confirm that the names of travelers are accurate. Tickets are non-transferable and name changes are not permitted. Passenger names must match government-issued photo ID exactly: Traveler 1 (N/A). (make changes)</p>
          <p>Total price shown includes all applicable taxes and our fees. Some airlines may charge additional baggage fees or other fees. Fares are not guaranteed until ticketed. PLEASE NOTE: THESE TICKETS ARE NON-REFUNDABLE AS PER AIRLINE RULES (see Fare Rules). Date and routing changes will be subject to airline penalties and our fees.</p>
        </div>
      </div>
      <div className="cm-tnc-section">
        ...
        <label htmlFor="tnc">
          ...
          HappytoBook.com's Privacy Policy & User Agreement
        </label>
      </div>
